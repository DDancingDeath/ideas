...
| [personal-agent-v0](./projects/apps/personal-agent-v0) | Weekend spike of [cross-device-personal-agent](./projects/apps/cross-device-personal-agent): type `notepad` on the phone → notepad opens on the dev box. One command kind, one device, shared-secret auth, in-memory storage, Expo Go. ~500 LoC, ≤12 h build target. | Idea — fully designed, code not started; ready to build. |
