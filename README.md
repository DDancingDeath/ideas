...
| [agent-guidelines-hub](./projects/agents/agent-guidelines-hub) | A single source of truth for my cross-project agent guidelines that propagates by default into every repo I own, so any agent starting work already follows my rules without me pasting them in each time. | Idea — captured 2026-06-20 (problem, design sketch, open decisions); no build yet, parked to work on later. |
