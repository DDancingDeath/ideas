...
>
> **⚠ marks a provisional value.** It is the best available starting point, but
> it has not been decided on its merits — see [Provisional values](#provisional-values).
> Build against it, but do not treat it as settled.
...
| `shopProfile.cash.mismatchLarge` | integer paise | `20000` (₹200) — **unsettled**, see [open questions](#open-questions) | owner | [`suspicion-engine.md`](./suspicion-engine.md) |
...
Five keys carry a ⚠ above. Each was written down by reading a **build attempt
that is not authoritative** — not by deciding the number on its merits. They
are recorded here rather than quietly trusted.

| Key | Provisional value | What to confirm |
| --- | --- | --- |
| `time.maxFutureMin` | `60` | How far ahead of server time may a device's claimed timestamp be before the event is rejected? An hour is generous for a shop phone. |
| `pricing.maxRateMultiple` | `5` | A sale rate more than 5× the item's moving average raises `price.unusually-high`. Is 5× the right sensitivity, or is it so loose it never fires? |
| `printer.maxRetries` | `3` (4 attempts) | How many times should a failed print retry before the job is marked `failed`? |
| `stock.negativeBlockMg` | `5_000_000` (5 kg) | How far below zero may computed stock go before a wholesale sale is blocked outright? |
| `stock.adjustmentLargeMg` | `20_000_000` (20 kg) | What size of manual stock adjustment is "large" enough to flag? |

Any of these can change without a schema migration, so they are safe to build
against in the meantime. Confirm them, then delete this section and the ⚠
markers.
...
- `TODO(spec, blocks: M8)` — **`shopProfile.cash.mismatchLarge`.** What
  ...
  `mismatchTolerance` flag? `plan/rebuild/decisions.md` M8 records ₹200. A
  prior build attempt used ₹500, but that carries no authority and no recorded
  reasoning, so it is not evidence for the larger figure. **Default:** keep the
  M8 value of `20000` (₹200) until the owner says otherwise — it is the only
  figure anyone actually signed off on. The ₹50 tolerance is not in dispute.
