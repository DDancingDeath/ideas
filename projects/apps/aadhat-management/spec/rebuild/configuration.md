...
| `shopProfile.time.maxFutureMin` | integer minutes | `5` | owner | [`time-clock.md`](./time-clock.md) — reject events claiming the future |
...
| `shopProfile.stock.negativeBlockMg` | integer milligrams | `required — no default` → [open questions](#open-questions) | owner | [`suspicion-engine.md`](./suspicion-engine.md) `stock.negative.large` |
| `shopProfile.stock.adjustmentLargeMg` | integer milligrams | `required — no default` → [open questions](#open-questions) | owner | [`suspicion-engine.md`](./suspicion-engine.md), [`event-schemas.md`](./event-schemas.md) |
...
| `shopProfile.pricing.maxDiscountPctByRole` | map `role → integer percent` | `{ staff: 5, manager: 15, owner: 100 }` | owner | [`suspicion-engine.md`](./suspicion-engine.md) |
| `shopProfile.pricing.maxRateMultiple` | decimal multiplier | `2.0` | owner | [`suspicion-engine.md`](./suspicion-engine.md) |
...
| `shopProfile.printer.maxRetries` | integer attempts | `5` (total, including the first) | owner | [`print-queue.md`](./print-queue.md), [`printer-compatibility.md`](./printer-compatibility.md) |
...
- `TODO(spec, blocks: M6)` — `shopProfile.stock.negativeBlockMg`: how far below
  zero may computed stock go before a wholesale sale is **blocked** outright?
  **Default:** `5_000_000` (5 kg) — small enough to catch a mis-keyed quantity,
  large enough to survive normal purchase-entry lag.
- `TODO(spec, blocks: M6)` — `shopProfile.stock.adjustmentLargeMg`: what
  magnitude of manual adjustment is "large" enough to flag? **Default:**
  `10_000_000` (10 kg).

Both are shop-scale judgements only the owner can confirm; the defaults above
are safe to build against and to change later without a schema migration.
